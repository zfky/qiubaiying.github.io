...
将远程仓库到修改同步到本地：  
 &ensp查看远程仓库：git remote -v（要cd到本地仓库到目录下）  
 &ensp将远程仓库到master分支下载到本地：git fetch origin master  
 &ensp比较本地仓库和远程的区别：git log -p master.. origin/master(按p退出)  
 &ensp下载下来到代码合并到本地仓库：git merge origin/master
